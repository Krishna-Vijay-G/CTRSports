...
import { PlusIcon } from "@/admin/ui/icons";
...
export function TracksEditor({ initialTracks }: { initialTracks: Track[] }) {
  ...
  return (
    <div className="mx-auto flex h-full max-w-3xl flex-col gap-2 overflow-y-auto pb-6">
      <div className="rounded-lg border border-border bg-card px-4 py-3">
        <h1 className="font-medium tracking-tight text-foreground">Circuits</h1>
        <p className="mt-0.5 text-[11px] text-muted-fg">
          The tracks the championship runs on. The calendar points at these, so a change here
          shows up on every round that visits.
        </p>
      </div>
      ...
      <div className="space-y-2.5 rounded-lg border border-border bg-card p-3">
        ...
        <Note>
          Racers and teams are not here on purpose. The lap record holder and the car it was set
          in belong to tables of their own, and this screen will point at them once those exist.
        </Note>
      </div>
    </div>
  );

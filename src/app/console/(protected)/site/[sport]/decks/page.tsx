 ...
 * One sport's decks.
 *
 * A deck used to belong to nobody — one global table, one global screen, and a
 * comment saying so. It belongs to a sport now, which is what stops a
 * pickleball co-admin opening the championship's media pack.
 *
 ...
 */
export default async function DecksAdminPage({ params }: Props) {
  const { sport } = await params;
  const { site } = await requireSite(sport, "decks");

  const [decks, chrome] = await Promise.all([listDecks(site.id), getChrome(site)]);

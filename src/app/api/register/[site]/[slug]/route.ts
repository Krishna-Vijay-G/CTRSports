...
/*
 * The SITE is in the path, not a query parameter.
 *
 * A form slug is only unique within a site — `ctr.slugs` is keyed on all three
 * columns — so `/api/register/<slug>` stopped naming one thing the moment there
 * could be two sports. It is the site's SLUG rather than its id because that is
 * what the page it was submitted from already has, and because an id in a public
 * URL is noise.
 */
...
/**
 * The site behind the path segment.
 *
 * 404 rather than 400 for a segment naming no site: a submission to an address
 * that does not exist is the same thing as a submission to a form that does not
 * exist, and the caller learns nothing either way. A DRAFT site is refused too —
 * its form has no public page, so nothing legitimate can be posting to it.
 */
async function siteIdOf(slug: string): Promise<string> {
...
export async function POST(request: Request, { params }: Params) {
  ...
  if (multipart) return postWithFiles(request, site, slug);
  ...
  return complete({ request, site, slug, ip, body, uploads: [] });
}
 ...
 */
async function postWithFiles(request: Request, site: string, slug: string) {
  const ip = callerIp(request);
  ...
  try {
    form = await getFormBySlug(await siteIdOf(site), slug);
  } catch (error) {
...
async function complete({
  ...
  request: Request;
  /** The site SLUG from the path — see the note on Params. */
  site: string;
  slug: string;
  ip: string;
  ...
  try {
    const form = await getFormBySlug(await siteIdOf(site), slug);

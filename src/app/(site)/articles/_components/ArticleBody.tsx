import type { ArticleDoc, ArticleInline, ArticleListItem, ArticleNode } from "@/lib/articles";
 ...
 * what follows is a `switch` over the set of types `ArticleNode` names. Every
 ...
 * and `link()` by `normaliseArticleBody` on the way into the database, and are
...
function Inlines({ nodes }: { nodes: ArticleInline[] }) {
  return (
    <>
...
function ListItems({ items }: { items: ArticleListItem[] }) {
  return (
    <>
...
function Blocks({ nodes }: { nodes: ArticleNode[] }) {
  return (
    <>
      {nodes.map((node, index) => {
        switch (node.type) {
          ...
          default:
             ...
             * adding a type to `ArticleNode` and forgetting to draw it renders
...
export function ArticleBody({ doc, className }: { doc: ArticleDoc; className?: string }) {

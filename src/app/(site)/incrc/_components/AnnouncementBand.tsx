...
export function AnnouncementBand({
  ...
  announcement: IncrcContent["announcement"];
  /**
   * Every published article, from ctr.articles. The card resolves its link
   * against these for the cover, the words and the date it does not carry.
   */
  articles: ArticleSummary[];
  /** Every published deck, from ctr.decks. Resolved the same way. */
  decks: DeckSummary[];
}) {
  /*
   ...
   */
  const articleSlug = slugFromArticleHref(announcement.href);
  const deckSlug = slugFromDeckHref(announcement.href);

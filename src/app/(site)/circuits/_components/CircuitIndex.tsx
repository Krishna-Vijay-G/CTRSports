 ...
 * brochure.
 */
export function CircuitIndex({ tracks }: { tracks: Track[] }) {
            ...
            {tracks.map((track, index) => (
              ...
              <li key={track.id} className="flex">
                <Reveal delay={index * 0.06} className="w-full">
                  <CircuitCard track={track} position={index + 1} />
                </Reveal>
              </li>
            ))}
          </ul>
...
export function CircuitCard({ track, position }: { track: Track; position: number }) {

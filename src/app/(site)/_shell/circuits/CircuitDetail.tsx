...
import { eventHref, type EventSummary } from "@/lib/events";
import { eventDateLabel } from "@/lib/raceDates";
import { circuitsHref, hostOf, majorEventList, trackHref, type Track, type TrackLink } from "@/lib/tracks";
 ...
 *
 * The header is the shell's, drawn above this: the masthead here is a bordered
 * panel inside the page's rhythm, not a full-bleed hero, so there is nothing to
 * lay a navigation bar over.
 *
 * `season` holds the weekends run here — rows of ctr.events, handed in rather
 * than reached for, because this component is also drawn inside the console's
 * circuit preview where nothing may touch the database. Each one links to its
 * own page, which is what migration 0018 made possible: a round used to be a
 * position in a list and had nowhere to go.
 */
/**
...
export function CircuitDetail({
  ...
  tracks: Track[];
  /**
   * The weekends of the season held HERE, for the "On the calendar" block.
   *
   * `season` rather than `events` because this component already had one: the
   * championships a circuit hosts are strings from `majorEventList`, and they
  ...
  const visits = season.filter((entry) => entry.track_id === track.id);
      ...
      <div className="shell py-6 lg:py-10">
                ...
                <section className="w-full border border-line bg-panel p-6">
                  ...
                  <ul className="mt-4 divide-y divide-line">
                    {visits.map((visit) => (
                      <li
                        key={visit.id}
                        className="relative flex items-center gap-4 py-2.5 first:pt-0"
                      >
                        ...
                        <span className="min-w-0 flex-1">
                          {/*
                            The weekend's own page, which is what 0018 made
                            possible: this used to be plain text, because a round
                            was a position in a list and had nowhere to go.
                            after:inset-0 makes the whole row the target.
                          */}
                          <Link
                            href={eventHref(site, visit)}
                            className="block truncate text-sm font-semibold text-fg transition-colors after:absolute after:inset-0 hover:text-accent"
                          >
                            {eventDateLabel(visit) || "Date to be confirmed"}
                          </Link>
                          {visit.badge ? (
                  ...
                  <Arrow href={`${siteHref(site)}#calendar`}>The full season</Arrow>
                </section>
              </Reveal>
            ) : null}
          </div>
        ...
        {previous || following ? (
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <NeighbourLink site={site} track={previous} direction="previous" delay={0.05} />
            <NeighbourLink site={site} track={following} direction="next" delay={0.09} />
          </div>
        ) : null}
      </div>
    </>

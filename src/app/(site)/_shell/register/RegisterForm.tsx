...
export function RegisterForm({
  ...
  form: Form;
  /**
   * The SLUG of the site this form belongs to, for the submit address.
   *
   * Defaulted, because the console preview mounts this component with no site
   * and never submits — the same reason `nonce` and `issuedAt` are optional.
   */
  site?: string;
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      // The site is in the path because a form slug is only unique within
      // one — see the note on the route's Params.
      const response = await fetch(`/api/register/${site}/${form.slug}`, {
        method: "POST",
        headers,
        body,
      });

...
import { deckHref, type DeckSummary } from "@/lib/decks";
import { MAX_DECK_CARDS, type DeckCard, type Decks } from "./model";
 ...
 */
export function DecksPanel({ value, onChange, ctx }: SectionPanelProps<Decks>) {
  const { decks } = ctx.records;
        ...
        summary={(card) => {
          ...
          return {
            title:
              card.title ||
              deck?.name ||
              (card.slug ? deckHref(ctx.records.site, card) : "No deck yet"),
            hint: deck
              ? `${deckHref(ctx.records.site, deck)} · ${deck.pages} ${deck.pages === 1 ? "page" : "pages"}`
              : card.slug

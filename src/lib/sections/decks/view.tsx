 ...
 */
export function DecksView({ value: copy, records }: SectionViewProps<Decks>) {
  // Every published deck of this site. Passed in — this may be a preview.
  const available = records.decks;
        ...
        {cards.map(({ card, deck }, index) => (
          ...
          <li key={deck.slug} className="flex">
            <Reveal delay={Math.min(index, 5) * 0.06} className="flex w-full">
              <Link
                href={deckHref(records.site, deck)}

...
import { ANNOUNCEMENT_INKS, DEFAULT_ANNOUNCEMENT_COLOUR, type AnnouncementInk, type Announcement } from "./model";
...
export function AnnouncementPanel({ value, onChange, ctx }: SectionPanelProps<Announcement>) {
  const { articles, decks } = ctx.records;
  ...
  const articleSlug = slugFromArticleHref(ctx.records.site, value.href);
  const deckSlug = slugFromDeckHref(ctx.records.site, value.href);

 ...
 * the preview at it — `focusBanner` reports the index up to the editor, which
 * holds the carousel still on that one for as long as it is open. Without that
 * the preview would keep rotating and you would be editing a banner you cannot
 * see.
 *
 * Everything here is part of the page, so it all goes with the one Save at the
 * top. Nothing in this panel writes on its own.
 */
export function BannersPanel({ value, onChange, ctx }: SectionPanelProps<Banners>) {
  const banners = value.items;
  const onFocus = ctx.focusBanner;
  const setItems = (items: Banner[]) => onChange({ items });
  ...
  function setBanner(id: string, patch: Partial<Banner>) {
    setItems(banners.map((banner) => (banner.id === id ? { ...banner, ...patch } : banner)));
  }
  ...
  function add() {
    ...
    // A fresh banner points at the top of the page it is on. It used to take
    // a per-page default from the editor; a section does not know which page
    // it is on, and "#top" is the one destination that is right on all of them.
    const banner = blankBanner(crypto.randomUUID(), "#top");
    setItems([...banners, banner]);
    open(banner.id);
  }

  function remove(id: string) {
    setItems(banners.filter((banner) => banner.id !== id));
